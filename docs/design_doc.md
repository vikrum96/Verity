...
**Validation set construction:** The human-labeled validation set contains 57 pairs — 50 organic outputs generated by DistilBART and 7 synthetic pairs constructed to ensure minimum calibration coverage across all five failure categories. The organic pairs reflect DistilBART's natural failure distribution on this dataset, which is dominated by completeness failures. The synthetic pairs introduce unambiguous hallucination and coherence failures that are underrepresented in the organic set, ensuring Gemini is calibrated against clear examples of each failure mode. Synthetic pairs are labeled in the validation set notes column and tracked separately in calibration analysis.
...
**Process:**
1. Run the Gemini judge against the 57-example human-labeled validation set (`data/validation_set.csv`), which includes 50 organic DistilBART outputs and 7 synthetic pairs constructed for calibration coverage.
